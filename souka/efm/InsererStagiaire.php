...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO stagiaires (nom, prenom, email, filiere, image) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $nom, $prenom, $email, $filiere, $target_file);
    $stmt->execute();
    $stmt->close();
...
$result = $conn->query("SELECT * FROM filieres");
